...
char *DataHandler::wifiPass(const char *wifiPass)
{
    ...
    if (wifiPass != nullptr && strlen(wifiPass) == 1 && wifiPass[0] == ' ')
    {
        Serial.println("II: WifiPassword was NOT updated");
        return EMPTY_STR;
    }
...
char *DataHandler::brokerPassword(const char *brokerPassword)
{
    ...
    if (brokerPassword != nullptr && strlen(brokerPassword) == 1 && brokerPassword[0] == ' ')
    {
        Serial.println("II: BrokerPassword was NOT updated");
        return EMPTY_STR;
    }
...
char *DataHandler::updateData(char *originalValue, const char *updateValue)
{
    ...
    {
        Serial.print("DD: Updating value: ");
...
JsonObject DataHandler::getData()
{
    ...
    buf["wifi-password"] = &EMPTY_STR[0];
    buf["broker-address"] = brokerAddress();
    buf["broker-user"] = brokerUser();
    buf["broker-password"] = &EMPTY_STR[0];
